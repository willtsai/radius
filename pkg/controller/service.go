...
func init() {
	...
	utilruntime.Must(radappv1alpha3.AddToScheme(scheme))
}
...
func (s *Service) Run(ctx context.Context) error {
	...
	mgr, err := ctrl.NewManager(s.Options.K8sConfig, ctrl.Options{
		...
		LeaderElection:         false, // TODO make this configurable.
		LeaderElectionID:       "c85b2113.rad.app",
	})
	if err != nil {
	...
	if s.TLSCertDir == "" {
		...
		//mgr.GetWebhookServer().Register("/mutate-apps-v1-deployment", &webhook.Admission{Handler: &podAnnotator{Client: mgr.GetClient()}})
		err = (&reconciler.RecipeWebhook{}).SetupWebhookWithManager(mgr)
		if err != nil {
			return fmt.Errorf("failed to setup %s webhook: %w", "Recipe", err)
		}
		err = (&reconciler.DeploymentWebhook{}).SetupWebhookWithManager(mgr)
		if err != nil {
			return fmt.Errorf("failed to setup %s webhook: %w", "Deployment", err)
		}
	}
